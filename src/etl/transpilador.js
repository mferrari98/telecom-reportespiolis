...
function sustituirMarcas(reporte, estampatiempo, contenido, cb) {
  ...
  reporte.forEach((item, i) => {
    contenido = contenido
      ...
      .replace(
        `NIVEL_${i}`,
        item.variable.nivel.valor === sindet ? "" : item.variable.nivel.valor
      )
      .replace(
        `CLORO_${i}`,
        item.variable.cloro.valor === sindet ? "" : item.variable.cloro.valor
      )
      .replace(
        `TURB_${i}`,
        item.variable.turbiedad.valor === sindet
          ? ""
          : item.variable.turbiedad.valor
      )
      .replace(
        `VOLDIA_${i}`,
        item.variable.voldia.valor === sindet ? "" : item.variable.voldia.valor
      );
  });
    ...
    .replaceAll(
      ...
      reporte.map((objeto) =>
        objeto.variable.nivel.valor != sindet
          ? (objeto.rebalse - objeto.variable.nivel.valor).toFixed(3)
          : 0
      )
    )
    .replaceAll(
      "<!-- REBALSE -->",
      reporte.map((objeto) => objeto.rebalse.toFixed(3))
    );
